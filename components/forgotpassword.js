...
export default function Forgotpassword() {
    ...
    const onSubmit = async (val) => {
        if (val.password.length === 0 || val.pas.length === 0) {
            ...
            let a = await fetch("/api/signup", { method: "POST", body: JSON.stringify({ email: val.email, password: val.password }), headers: { 'content-type': 'application/json' } })
            let b = await a.json();
            if (b) {
                toast('Password is changed', { position: "top-right", autoClose: 3000 })
                navigate("/")
                ref.current.reset()
            }
        }
    ...
    return (<>
        ...
        <div className="absolute top-1 flex justify-center items-center content-between flex-col w-full ">
            ...
            <form className=" w-[400px] p-5 text-white rounded-xl my-5 shadow-[0px_0px_20px_5px] shadow-black backdrop-brightness-50" onSubmit={handleSubmit(onSubmit)} ref={ref} action="./api/data" method="post">
                <div className="text-[1.5rem]">FORGOT PASSWORD</div>
                <div className="my-5">
                    <p>Email Address</p>
                ...
                <button type="submit" className="my-5 w-full h-[40px] bg-orange-600 font-bold text-white">CHANGE PASSWORD</button>
